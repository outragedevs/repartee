...
| G2 — authentication paths and TLS rejection | Explicit PASS-before-BIND now works with Lurker, with exact registration confirmation and disk-backed history exclusion. Actual-provider fixtures verify PASS control mode on both, Soju BIND refusal, missing/invalid credentials and invalid PLAIN on both. See [PASS evidence](bouncer-pass-authentication.md). Legacy username/network selector routes still need correct history ownership and identity checks, including provider-aware scope continuity when a Soju PASS-only secret contains a colon. Actual untrusted/wrong-host server TLS rejection also remains open; OAuth and revoked EXTERNAL tests do not prove these cases. |
...
2688 native tests and 146 web tests passed, with 27 native provider fixtures
