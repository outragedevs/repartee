...
`a0ced811e69cb99d456a3489b1f853562ce16a08` (merged PR 121).
...
| Presence | `src/app/presence.rs`, `src/app/presence_fixture.rs::pinned_bouncer_presence`: two actual Apps, preregistration absence, manual away, reconnect, Soju AutoAway on/off and Lurker account-wide semantics. Actual headed browser focus/visibility transitions and upstream AWAY are verified on both providers; native attach/detach remains open. [Presence](bouncer-presence.md). | [71](https://github.com/outragedevs/repartee/pull/71) | G5, G6 |
...
| File upload | `src/app/filehost_fixture.rs::pinned_bouncer_filehost`, backend/client upload checks and actual browser picker/provider runs. OPTIONS/POST, returned URL and transport protections are recorded in [FILEHOST](bouncer-filehost.md). | [83](https://github.com/outragedevs/repartee/pull/83) | G5 |
...
Open rows are required work, not waived limitations. G1, G2, G4, G7 and G8 have
recorded acceptance evidence; G3, G5, G6 and G9 remain open. “No evidence found” means
