    ...
    fn negotiate_with_full_desired_list() {
        let caps = ServerCaps::parse(
            "multi-prefix extended-join ircnet.com/extended-join server-time \
             ...
             echo-message invite-notify batch draft/extended-isupport labeled-response userhost-in-names no-implicit-names draft/no-implicit-names soju.im/no-implicit-names message-tags \
