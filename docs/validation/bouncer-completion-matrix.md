...
| G4 — full history query family | `/bsearch between` now provides tracked two-bound requests through native and web command handlers, with clamped limits and shared cancellation/quarantine. The [bounded history acceptance](bouncer-history-ranges.md) verifies both providers' wire semantics and actual App results with SQLite exclusion. Actual WebKit covers both directions and reload. In-flight interrupted range acceptance remains open. |
