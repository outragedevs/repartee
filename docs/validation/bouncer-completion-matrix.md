...
`88a859f73b0febd3fde4ff9274418f7f58457a46` (merged PR 107).
...
contract. In particular, the advertised `soju.im/SAFERATE` still has no consumer
on this baseline; G1 is an implementation/release gap.
...
| --- | --- |
| G1 — SAFERATE and distributable dependency | Main has no SAFERATE consumer. The separate `irc-repartee` 1.5.2 release and root implementation remain outside this baseline. Publish the reviewed library after local registry authentication, switch Repartee to the published dependency, review/merge its runtime flood-control change, and verify builds through the intended `cargo install` route. Do not merge the temporary vendor workaround. |
| G2 — authentication paths and TLS rejection | Explicit PASS-before-BIND now works with Lurker, with exact registration confirmation and disk-backed history exclusion. Actual-provider fixtures verify PASS control mode on both, Soju BIND refusal, missing/invalid credentials and invalid PLAIN on both. See [PASS evidence](bouncer-pass-authentication.md). The current draft detects legacy USER/PASS network selectors on the actual connection, confirms runtime identity, suppresses autojoin and excludes conversation history from SQLite; both pinned providers pass the persistent-history/reconnect fixture. See [implicit registration](bouncer-implicit-registration.md). Registry dependency publication, implicit control child connections, non-SASL privileged feature eligibility and provider-aware scope continuity for colon-bearing PASS secrets remain open. Actual TLS cases on both endpoints now verify a successful trusted connection, UnknownIssuer rejection and wrong-host rejection, with no successful App/web connection state on failure; [TLS acceptance](bouncer-tls-validation.md). |
| G3 — complete target discovery | Actual tests prove a 1001-way timestamp tie omits one name, show a warning, and recover by a known name. They do not discover that unknown name. Both providers filter closed/detached targets after LIMIT; the filtered-empty runtime case remains unproved. Reproduce it and establish a complete supported retrieval strategy, or identify the precise upstream protocol change/user decision required. A warning alone is not full enumeration. |
| G4 — full history query family | `/bsearch between` now provides tracked two-bound requests through native and web command handlers, with clamped limits and shared cancellation/quarantine. The [bounded history acceptance](bouncer-history-ranges.md) verifies both providers' wire semantics and actual App results with SQLite exclusion. Actual WebKit covers both directions and reload. The [partial history fixture](bouncer-partial-history.md) covers disconnect after the first actual range row and clean retry. Stalled range timeout and explicit cancellation now release and discard actual late bytes before a clean retry. Real 60/90-second expiry variants verify late orphan discard and reconnect-required recovery. Concurrent isolation acceptance remains open. |
...
| G8 — interrupted history/search batches | Existing unit tests cover timeouts, cancellation, late/orphan rows and scoping. PR 107 adds real TCP loss after completed operations. The [partial history App fixture](bouncer-partial-history.md) now proves an actual BETWEEN prefix, disconnect, discard, clean retry and SQLite exclusion on both providers. Add bounded actual-daemon evidence for an interrupted or timed-out in-flight history/search response, then inspect storage/diagnostics and recovery. Do not relabel the completed-operation disconnect as a partial-batch test. |
...
2688 native tests and 146 web tests passed, with 28 native provider fixtures
