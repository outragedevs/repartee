...
| G2 — authentication paths and TLS rejection | Explicit `bouncer_network_id` currently rejects registration without successful SASL in `src/irc/mod.rs`. Lurker additionally accepts supplied PASS before BIND; Soju does not. Implement or prove the supported PASS/selector routes with correct history ownership, credential isolation and clear rejection of invalid combinations. Add real client checks for untrusted/wrong-host server TLS failures and invalid PLAIN login on both endpoints; OAuth and revoked EXTERNAL tests do not prove these cases. |
